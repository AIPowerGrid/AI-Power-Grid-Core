...
cd ${GITHUB_WORKSPACE}
PKGVERSION=`grep "PACKAGE_VERSION" src/config/aipg-config.h | cut -d\" -f2`
VERSION="${PKGVERSION}"
...
if [[ ${GITHUB_BASE_REF} =~ "release" ]]; then
    DISTNAME="AI-Power-Grid-${VERSION}"
else
    DISTNAME="AI-Power-Grid-${VERSION}-${SHORTHASH}"
fi
...
if [[ ${OS} == "windows" ]]; then
    ...
    if [[ -e ${GITHUB_WORKSPACE}/dist/Aipg-Qt.app/Contents/MacOS/install_cli.sh ]]; then
        chmod +x ${GITHUB_WORKSPACE}/dist/Aipg-Qt.app/Contents/MacOS/install_cli.sh
    fi
    ...
    ${GITHUB_WORKSPACE}/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Aipg-Core.dmg" ${RELEASE_LOCATION}/${DISTNAME}-osx-unsigned.dmg
