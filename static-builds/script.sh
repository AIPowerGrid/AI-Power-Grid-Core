...
find ${DISTNAME}/bin -type f -executable -exec ../Aipg/contrib/devtools/split-debug.sh {} {} {}.dbg \;
find ${DISTNAME}/lib -type f -exec ../Aipg/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
cd ~/
rm -rf win64
cp -rf Aipg/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir -p unsigned
mv ~/Aipg/aipg-*setup-unsigned.exe unsigned/
...
cd ~/
rm -rf win32
cp -rf Aipg/contrib/windeploy ~/sign/win32
...
mkdir -p ~/Aipg/depends/SDKs
cp ~/MacOSX10.11.sdk.tar.gz ~/Aipg/depends/SDKs/MacOSX10.11.sdk.tar.gz
cd ~/Aipg/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
