...
The RPM spec file provided here is for Aipg-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://aipgcrypto.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Aipg git repository](https://github.com/JustAResearcher/Aipg) but are not part of
...
Currently only `Source10` is used, for the example `aipg.conf` file.
...
in the aipg git repository. At present only an SVG version of the Aipg
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the aipg
...
needed when 0.12.1 is released, a proper fix is already in the Aipg git
master branch.
...
idea to build Aipg Core against a static version of that library compiled
...
The `aipg.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `aipg-server` package. At this point, most RPM based
...
The `aipg-server` package should have SELinux support. How to properly do
...
Aipg-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/aipg/). He has been
packaging Aipg for Fedora at least since 2012.
...
4. I renamed the `aipg` package that contains the Qt GUI to `aipg-core` as
that appears to be how the general population refers to it, in contrast to
`aipg-xt` or `aipg-classic`. I wanted to make sure the general population
