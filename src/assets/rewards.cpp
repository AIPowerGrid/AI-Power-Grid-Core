...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for aipg
    ...
    if (p_rewardSnapshot.strDistributionAsset != "aipg") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is aipg with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using aipg differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "aipg") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Aipg address (already validated during ownership snapshot creation)
